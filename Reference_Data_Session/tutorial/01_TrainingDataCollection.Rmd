---
title: "Collection of training data for model building"
subtitle: "Tutorial: EON Summer School 2022"
...
In this tutorial we will work with a Sentinel-2 scene from 18/06/2022 from the National Park, Harz. We will also use the boundary of the National Park to define our study area. Before we can start you may download the S2 Scene from the following link:
[S2-download](https://cloud.hawk.de/index.php/s/gyPKzMYP63tdQKZ/download/S2B_MSIL2A_20220618T102559_N0400_R10_resampled_harz_np.tif). Place this file into the data folder of this tutorial. 
...
s2  <- raster::brick("data/S2B_MSIL2A_20220618T102559_N0400_R10_resampled_harz_np.tif")
...
From the output of the PCA we see that we can capture 92% of the variability with the two components. Thus we will only use the PC1 and PC2 for the subsequent analysis.

In the next step we run an unsupervised classification of the PC1 and PC2 to get a clustered map. For the unsupervised classification we need to take a decison on the number of classes/clusters to be created. Here we will take n=5 classes. However, depending on the target variable this value need to be adjusted.
...
# Implement a plot design and extract the pixel values at the samples
...
# Check the pixel value range from the sample and the image
